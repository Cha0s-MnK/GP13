...
# get selected files
file_list = sorted(glob.glob(rms_files))
...
for file_id, file in enumerate(file_list[:5]):
    ...
    for entry in range(num_entries):
        ...
        for channel_id, channel in enumerate(channels):
            ...
            window_list = search_windows(filtered_trace, num_threshold*noises[channel_id], standard_separation)
...
for du in du_list:
    ...
    rms_file = os.path.join(rms_dir, rms_name)
    np.savez(rms_file, **{f'noise_list{channel}': noise_list[channel][du] for channel in channels})
    print(f'Saved: {rms_file}')