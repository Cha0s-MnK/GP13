...
def plot_du_amplitude(file_list):
    ...
    amplitude_plot_file = os.path.join(amplitude_plot_dir, f'amplitude_DU{du}_{datetime_flat}.png')
...
def plot_channel(ax,
    ...
    amplitude_frequencies = amplitude_nums / len(amplitude_list) * 5e8
    ...
    sigma = np.std(amplitude_list)
    normal_distribution = norm.pdf(bins, 0, sigma)
    frequency_normal_distribution = normal_distribution * 5e8
...
def main():
    ...
    files_str = amplitude_npz_dir + f'/amplitude_DU10*_20231118031953.npz'
    ...
    for du in du_list:
        # get the string of NPZ files for this DU
        files_str = amplitude_npz_dir + f'/amplitude_DU{du}_20231118031953.npz'
