...
def get1entry_trace(num_run, entry, tadc, trawv):
    ...
    if du_list != False and du not in du_list:
        return
    ...
    if wanted == 'pulse':
        num_pulse = 0
        for channel in channels[:2]: # exclude channel Z
            window_list = search_windows(trace=filter_traces[channel], filter_status='off')
